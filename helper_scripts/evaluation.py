...
parser.add_argument("--output_path", help="Path where the files are output", type=str, required=False)
...
def compare_features(gt_features, ocr_features):
    ...
    ocr_used = [False] * len(ocr_features)

    for idx, ocr in enumerate(ocr_features):
        match_found = False
        for jdx, gt in enumerate(gt_features):
            if gt_used[jdx]:
                continue
            if gt["type"] == ocr["type"]:
    ...
    # Any OCR feature that is not present in GT, an extra
    for idx, used in enumerate(ocr_used):
        if not used:
            extras.append(ocr_features[idx])
...
def main():
    ...
    # This will only be used for the comparison, for gemma3
    ocr_no_backticks = re.sub(r'`+', '', ocr_markdown, flags=re.DOTALL)
    ...
    c = compare_features(extract_features(ground_truth, "gt"), extract_features(ocr_no_backticks, "llm"))
    c["CER"] = CER
    c["WER"] = WER
    c["prompt_token_count"] = prompt_token_count
    c["completion_token_count"] = completion_token_count
    c["total_tokens"] = total_tokens
    
    # with open("gt.md", 'w', encoding='utf-8') as f:
    ...
    # output_name = f"eval_{Path(args.ocr_output).name.rstrip("json")}"
